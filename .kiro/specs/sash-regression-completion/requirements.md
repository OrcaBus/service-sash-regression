...
- **PublisherFunction**: Lambda that receives `SashRegressionComparisonCompleted` and posts a
  result summary to both GitHub (new issue) and Slack (Incoming Webhook).
...
**User Story:** As an operator, I want the comparison result posted to GitHub as a new issue, so
that the release decision-maker sees regression status without leaving GitHub.
...
2. THE PublisherFunction SHALL create a new issue on `umccr/sash` via
   `POST /repos/umccr/sash/issues`, titled with the outcome and version comparison, containing
   per-status pair counts, critical items (if present), a link to the results in S3, and a link
   to `https://github.com/umccr/sash/tree/release/{newVersion}` (branch-naming convention
   assumed to hold — verified across every sash release in git history: 0.6.0, 0.6.1, 0.6.2,
   0.6.4, 0.7.0 — not re-verified via API call at runtime).
...
Posting a new issue (rather than a PR comment) means there is no PR-lookup step and therefore no
"is the PR still open" race condition to handle — every run always succeeds in creating an
issue, independent of the state of any release branch or PR.
...
3. THE test suite SHALL include `test_publisher_github.py` covering issue formatting and
